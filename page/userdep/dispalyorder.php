...
if (isset($_GET['id_Done'])){
  $sq=$connect->prepare("UPDATE requirestorder SET status=? WHERE id=? AND Type=?");
...
//validaion needs
     if (isset($_GET['saveord'])){
         ...
         $goal=$_GET['goal'];
         $have=$_GET['have'];
         $sql=$connect->prepare("INSERT INTO requirestorder (id_user,name_applay,phone_applay,name_po,des,status,goal,havemo,Type)VALUES (?,?,?,?,?,?,?,?,?)");
         if ($sql->execute(array($id_user,$name_ap,$phone_ap,$name,$comm,'accept',$goal,$have,"pk"))){
 ...
 <div class="col-sm-6 col-sm-offset-5">
 	...
 	<div class="col-xs-10 col-sm-offset-1">
        ...
        <th>المحصول</th>
 			...
 			<tbody>
 				<?php
$sql=$connect->prepare("SELECT * FROM requirestorder WHERE Type =? OR id_user=?");
             $sql->execute(array("pk",$id_user));
           $su="";
           $ids='';
             foreach ($sql as $row) {
             if ($row['status']=="new"){
