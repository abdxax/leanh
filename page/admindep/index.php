...
if (isset($_POST['save'])){
	...
	$user=$_SESSION['id'];
	if ($pass1==$pass2){
		$res=$connect->prepare("INSERT INTO user (name,email,password,id_admin,phone,dep)VALUES (?,?,?,?,?,?)");
		if ($res->execute(array($name,$email,$pass1,$user,$phone,"yes"))){

$res=$connect->prepare("INSERT INTO userAll (email,password,role)VALUES (?,?,?)");
         if($res->execute(array($email,$pass1,"user"))){
...
  </div>
				...
				<a href="listofVoluntary.php">
					<div class="panel panel-default">
						...
						<div class="panel-body" >عرض الموظفين </div>
					</div>
					</a>
				</div>
				...
				<a href="static.php">
					<div class="panel panel-default">
						...
						<div class="panel-body">الانجازات </div>
					</div>
					</a>
				</div>
